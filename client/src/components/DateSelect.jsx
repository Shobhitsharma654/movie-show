    ...
    <div className='flex items-center gap-6 text-sm mt-5'>
        ...
        <span className='grid grid-cols-3 md:flex flex-wrap md:max-w-lg gap-4'>
            {Object.keys(dummyDateTimeData).map((date) =>(
                <button onClick={()=> setSelected(date)} key={date} className={`flex items-center justify-center flex-col h-14 w-14 aspect-square rounded cursor-pointer ${selected === date ? "bg-primary text-white" :"border border-primary/70"}`}>
